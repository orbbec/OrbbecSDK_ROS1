...
```
或者你可以使用以下脚本来获取序列号和usb端口号，把以下脚本保存为`list_ob_devices.sh`，`chmod +x list_ob_devices.sh`，然后运行`./list_ob_devices.sh`。
...
| astra mini          | astra_mini.launch     |
| astra mini pro      | astra_mini_pro.launch |
| astra pro           | astra_pro.launch      |
