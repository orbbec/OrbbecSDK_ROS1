 ...
 */
typedef struct _NvBufferCDEParams
{
...
* @param[out] syncobjparams  sync object parameters having size NVBUF_MAX_SYNCOBJ_PARAMS * sizeof(NvBufferSyncObjParams).
* @param[out] nparams        number of sync object parameters.
*
* @returns 0 for success, -1 for failure
*/
 ...
 *
 * @param[out] dmabuf_fd     Returns the DMABUF FD of the hardware buffer.
 * @param[in]  input_params  Input parameters for comprssed hardware buffer creation.
 *
 * @returns 0 for success, -1 for failure
 */
...
/**
 * Imports DMA buffer FD received from another process and create hardware buffer using the
 * input params to be used by calling process. As of now physicaladdress, payloadmetaInfo, syncobj,
 * nv_buffer, nv_buffer_size fields of NvBufferParamsEx are not required for this API so it can be zeroed.
 * Rest all fields of NvBufferParamsEx needs to be filled properly for this API.
 *
 * Calling process need to use Output DMA buffer FD for supported operations. After the intended use of the
 * imported DMA buffer FD, Calling process must need to call NvBufferDestroy() to remove reference count for
 * hardware buffer handle of the imported DMA buffer FD. Also input hardware DMABUF FD received from another
 * process needs to be closed.
 *
 * @param[in]  in_dmabuf_fd     Input hardware DMABUF FD received from another process.
 * @param[out] out_dmabuf_fd    Returns the Output hardware DMABUF FD to be used with calling process.
 * @param[in]  in_params        Extended input parameters for a hardware buffer.
 *
 * @returns 0 for success, -1 for failure
 */
 ...
 * @returns 0 for success, -1 for failure
 */
int NvBufferImportGraphicBufferFd(int *out_dmabuf_fd, buffer_handle_t *handle, NvBufferParamsEx *exparams);
#endif
