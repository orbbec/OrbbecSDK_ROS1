...
void printHelp() {
  std::cout
      << "Usage:\n"
      << "  rosrun orbbec_camera ip_config_tool <set_ip|force_ip|set_dhcp_timeout> "
         "[options]\n"
      << "      [--sdk_log_level debug]\n"
      << "Subcommands:\n"
      ...
      << "\n"
      << "  [Set IP]\n"
      << "    Persistent only: rosrun orbbec_camera ip_config_tool set_ip \\\n"
      ...
      << "                     --mask 255.255.255.0 \\\n"
      << "                     --gateway 192.168.1.1\n\n"
      << "    DHCP only:       rosrun orbbec_camera ip_config_tool set_ip \\\n"
      << "                     --current_ip 192.168.1.10 \\\n"
      << "                     --enable_dhcp true \\\n"
      << "                     --enable_persistent_ip false\n\n"
      << "    Both enabled:    rosrun orbbec_camera ip_config_tool set_ip \\\n"
      << "                     --current_ip 192.168.1.10 \\\n"
      << "                     --enable_dhcp true \\\n"
      << "                     --enable_persistent_ip true \\\n"
      << "                     --new_ip 192.168.1.200 \\\n"
  ...
  std::cout << "\n"
            ...
            << "  [SDK Log]\n"
            << "    Debug:   rosrun orbbec_camera ip_config_tool set_ip \\\n"
  ...
  try {
    ...
    if (args.operation == CliArgs::Operation::SET_IP) {
      ...
      if (v2_supported) {
        ...
        ROS_INFO("Set-ip target flags: %u.", ip_config_v2.flags);
        ROS_INFO("Set-ip target DHCP: %s.", args.dhcp ? "enabled" : "disabled");
        ROS_INFO("Set-ip target persistent IP: %s.", args.persistent_ip ? "enabled" : "disabled");
        ROS_INFO("Set-ip target persistent IP: %d.%d.%d.%d", ip_config_v2.address[0],
        ...
        if (args.dhcp == args.persistent_ip) {
          ROS_ERROR(
              "Legacy IP config (1041) supports mutually exclusive DHCP and persistent IP "
              "only. Use --enable_dhcp true --enable_persistent_ip false or "
        ...
        ROS_INFO("Set-ip target mode: %s (dhcp=%u).", ip_config.dhcp ? "DHCP" : "persistent",
                 ip_config.dhcp);
        ROS_INFO("Set-ip target DHCP: %s.", args.dhcp ? "enabled" : "disabled");
        ROS_INFO("Set-ip target persistent IP: %s.", args.persistent_ip ? "enabled" : "disabled");
        ROS_INFO("Set-ip target persistent IP: %d.%d.%d.%d", ip_config.address[0],
    ...
    if (args.operation == CliArgs::Operation::FORCE_IP) {
      ...
      if (context->forceIp(args.force_ip_mac.c_str(), ip_config)) {
        ...
        if (args.dhcp) {
          ...
          ROS_INFO("Force-ip target persistent IP: %s", args.new_ip.c_str());
