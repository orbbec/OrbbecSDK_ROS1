...
void OBCameraNodeDriver::init() {
  ...
  uvc_backend_ = nh_private_.param<std::string>("uvc_backend", "auto");
  auto uvc_backend = uvc_backend_;
  std::transform(uvc_backend.begin(), uvc_backend.end(), uvc_backend.begin(),
                 [](unsigned char c) { return std::tolower(c); });
  if (uvc_backend == "auto") {
    ROS_INFO_STREAM("Use default UVC backend from SDK config");
    ...
    ROS_WARN_STREAM("Unsupported uvc_backend " << uvc_backend_
