...
#include "orbbec_camera/point_cloud_proc/point_cloud_xyzrgb.h"
...
void PointCloudXyzrgbNode::imageCb(const sensor_msgs::ImageConstPtr& depth_msg,
  ...
  if (depth_msg->encoding == enc::TYPE_16UC1) {
    valid_count = convert<uint16_t>(depth_msg, rgb_msg, cloud_msg, red_offset, green_offset,
                                    blue_offset, color_step);
  } else if (depth_msg->encoding == enc::TYPE_32FC1) {
    valid_count = convert<float>(depth_msg, rgb_msg, cloud_msg, red_offset, green_offset,
                                 blue_offset, color_step);
  } else {
  ...
  pcd_modifier.resize(valid_count);
...
template <typename T>
size_t PointCloudXyzrgbNode::convert(const sensor_msgs::ImageConstPtr& depth_msg,
